...
LONG WINAPI gowvr::global_exception_handler(struct _EXCEPTION_POINTERS* ei) {
    ...
    if (dbghelp) {
        const auto final_path = Framework::get_persistent_dir("vrframework_crash_" + Framework::session_stamp() + ".dmp").string();
...
void gowvr::setup_exception_handler() {
